...
# フロントで閲覧 (静的配信 + 🤖 GPT精読APIを兼ねるローカルサーバ)
uv run scripts/server.py            # → http://localhost:8765/
```
...
| `scripts/deep_read.py` | 1論文を PDF DL → GPT 精読 → `outputs/deep_reads/<id>.json` 書き出し (ライブラリ兼CLI) |
| `scripts/server.py` | FastAPI: 静的配信 + 🤖 GPT精読 API (127.0.0.1:8765) |
| `outputs/` | 生成CSV置き場（gitignore済）/ `*.meta.json`・`deep_reads/*.json` はビューワーが読む |
...
`scripts/server.py` は FastAPI で静的配信 + `🤖 GPT精読` API を兼ねます。`python -m http.server` でも静的配信は可能ですが、その場合精読ボタンは使えません (既存の精読結果は閲覧可能)。
...
ビューワーで論文を行展開し、`🤖 GPT精読を実行` ボタンを押すと PDF を GPT に精読させて日本語の構造化サマリー (Standard フォーマット) を生成します。

### フォーマット (`outputs/deep_reads/<id>.json` の `sections`)

| フィールド | 内容 |
|---|---|
| `tldr` | 要点 1〜2文 |
| `problem_motivation` | 課題と重要性 (3〜5文) |
| `proposed_method` | 提案手法の構造 (5〜10文) |
| `key_results` | 主要結果の箇条書き (3〜6項目、数値付き) |
| `strengths_weaknesses` | `{strengths: [], weaknesses: []}` |
| `related_work_diff` | 既存研究との差分 (具体名つき) |

### 運用フロー

| 場所 | できること |
|---|---|
| ローカル (`uv run scripts/server.py` 起動中) | 新規精読の生成 + 既存結果の閲覧 |
| ローカル (`python -m http.server` 等、API無し) | 既存結果の閲覧のみ |
| 公開URL (GitHub Pages) | 既存結果の閲覧のみ (commit済みのものに限る) |

「自宅で精読 → push → 出先からスマホで公開URLを開く」運用を想定しています。`outputs/deep_reads/*.json` は git で追跡されます (`.gitignore` のホワイトリスト)。
...
```bash
uv run scripts/deep_read.py 2605.05328               # 結果は outputs/deep_reads/2605.05328.json
uv run scripts/deep_read.py 2605.05328 --overwrite   # 既存結果を無視して再生成
```

### セキュリティ

`scripts/server.py` は **127.0.0.1 のみ bind**。LAN/公開に晒さないでください — `/api/deep-read` を叩かれると OpenAI 課金が発生します。クラウドにデプロイする場合は認証が必要ですが、本リポジトリは「自宅で生成 → commit → 公開URLで閲覧」のローカル運用前提です。
